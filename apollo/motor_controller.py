...
class MotorController(object):
    ...
    def __init__(self, loop=None, simulate=False):
        ...
        self._state_dict = {
            ...
            'reported_pos': {'X': 0.0, 'Y': 0.0, 'Z': 0.0, 'A': 0.0, 'B': 0.0,
                             'C': 0.0},
            'actual_pos': {'X': 0.0, 'Y': 0.0, 'Z': 0.0, 'A': 0.0, 'B': 0.0,
                           'C': 0.0},
            'error': {'X': 0.0, 'Y': 0.0, 'Z': 0.0, 'A': 0.0, 'B': 0.0,
                      'C': 0.0},
            'absolute_mode': True,
            'feedback_on': False,
        ...
        self._config_dict = {
            ...
            'ack_ready_value': "0",
            'slack': {'X': 0.5, 'Y': 0.5, 'Z': 0.0, 'A': 0.1, 'B': 0.1,
                      'C': 0.0},
            'timeout': 5
        }
        ...
        self._move_list = [self.ABSOLUTE, self.RELATIVE, self.MOVE,
                           self.MOVE_TO, self.LINEAR_MOVE,
                           self.RAPID_LINEAR_MOVE]
        ...
        for k, v in kwargs.items():
            ...
            if K in self._state_dict['actual_pos'] and code in self._move_list:
                ...
                if self._state_dict['absolute_mode'] == True:
                    ...
                    # SWIT NEG
                    if aim_v_float < self._state_dict['actual_pos'][K] and \
                                    self._state_dict['direction'] == 0:
                        ...
                        self._state_dict['error'][K] = \
                        self._config_dict['slack'][K]

                    # SWIT POS
                    elif aim_v_float > self._state_dict['actual_pos'][K] and \
                                    self._state_dict['direction'] == 1:
    ...
    def _send(self, session_id=None, where_from=None, code=None, **kwargs):
        ...
        if session_id and where_from:
            ...
            if len(args) > 0:
                if code:
                    command = code + " " + ' '.join(args) + self._config_dict[
                        'message_ender']
                else:
                    command = ' '.join(args) + self._config_dict[
                        'message_ender']
            else:
            ...
            if self._state_dict['simulation'] == True:
                ...
                if self._transport is not None:
                    self._timeout_handler = self._loop.call_later(
                        self._config_dict['timeout'],
                        self._on_timeout_handler())
    ...
    def _send_command(self, session_id=None, where_from=None, code='',
                      **kwargs):
        """ Adds command to COMMAND QUEUE and then tries to step command queue
        """
        if session_id is not None and where_from is not None:
            cmd = {'session_id': session_id, 'code': code,
                   'where_from': where_from, 'args': kwargs}
    ...
    def _format_text_data(self, text_data):
        ...
        while remainder_data.find(',') >= 0:
            message_dict = self._format_group(
                remainder_data[:remainder_data.find(',')])
            ...
            if key == 'None':
                ...
                for k, v in value.items():
                    ...
                    if k in self._state_dict['reported_pos']:
                        ...
                        self._state_dict['actual_pos'][k] = aim_v_float + \
                                                            self._state_dict[
                                                                'error'][k]
                        axis_found = True
    ...
    def _process_message_dict(self, message_dict):
        ...
        if axis_found == True:
            ...
            if self._on_data:
                self._on_data(
                    session_id=self._state_dict['current_command_session_id'],
                    where_from=self._state_dict['current_command_where_from'],
                    message=pos_dict)
        ...
        if self._config_dict['ack_received_message'] in message_dict or \
                        self._config_dict['ack_received_message'] is None:
            ...
            if isinstance(value, dict):
                if self._config_dict['ack_receieved_parameter'] is None:
                    ...
                    for value_name, value_value in value.items():
                        if value_name == self._config_dict[
                            'ack_received_parameter']:
                            if self._config_dict[
                                'ack_received_value'] is None or value_value == \
                                    self._config_dict['ack_receieved_value']:
                                self._state_dict['ack_received'] = True
                                if self._timeout_handler is not None:
                                    self._timeout_hadler.cancel()
            else:
                if self._config_dict['ack_received_parameter'] is None:
                    if self._config_dict[
                        'ack_received_value'] is None or value == \
                            self._config_dict['ack_received_value']:
                        self._state_dict['ack_received'] = True
                        if self._timeout_handler is not None:
                            self._timeout_hadler.cancel()

        # 4. check if ack_ready confirmation
        if self._config_dict['ack_ready_message'] in message_dict or \
                        self._config_dict['ack_ready_message'] is None:
                ...
                if self._config_dict['ack_ready_parameter'] is None:
                    ...
                    for value_name, value_value in value.items():
                        if value_name == self._config_dict[
                            'ack_ready_parameter']:
                            if self._config_dict[
                                'ack_ready_value'] is None or str(
                                    value_value) == str(
                                    self._config_dict['ack_ready_value']):
                                self._state_dict['ack_ready'] = True
                            else:
                                self._state_dict['ack_ready'] = False
            else:
                if self._config_dict['ack_ready_parameter'] is None:
                    if self._config_dict['ack_ready_value'] is None or value == \
                            self._config_dict['ack_ready_value']:
                        self._state_dict['ack_ready'] = True
                    else:
                        self._state_dict['ack_ready'] = False
    ...
    def _on_connection_made_handler(self):
        ...
        if self._on_connection_made:
            self._on_connection_made(
                session_id=self._state_dict['connected_session_id'])
    ...
    def move(self, session_id, where_from="", **kwargs):
        ...
        self._send_command(session_id=session_id, where_from="where_from",
                           code=self.MOVE, **kwargs)

    def move_to(self, session_id, where_from="", **kwargs):
        """ Absolute linear move (G91 G0) """
        self._send_command(session_id=session_id, where_from="where_from",
                           code=self.MOVE_TO, **kwargs)
    ...
    def rapid_linear_move(self, session_id, where_from="", **kwargs):
        ...
        self._send_command(session_id=session_id, where_from="where_from",
                           code=self.RAPID_LINEAR_MOVE, **kwargs)
